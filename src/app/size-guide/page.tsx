      ...
      <div className="container mx-auto px-4 py-12 flex-1">
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {sizes.map((size) => (
              <div key={size.name} className="bg-white/80 dark:bg-card/80 backdrop-blur-sm rounded-lg p-5 shadow-sm text-center">
                <h2 className="text-xl font-bold text-main mb-1">{size.name}</h2>
                <p className="text-gray-600 dark:text-gray-400 text-sm">{size.dimensions}</p>
              </div>
            ))}
          </div>
          <div className="mt-8 bg-white/80 dark:bg-card/80 backdrop-blur-sm rounded-lg p-6 shadow-sm">
